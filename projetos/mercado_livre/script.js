...
const sumProducts = async () => {
  ...
  totalPrice.innerText = `O valor total de sua compra é: ${pruductsSum} reais`;
...
function createCartItemElement({ id: ItemID, title: name, price: salePrice }) {
  ...
  li.innerHTML = `SKU: ${ItemID} | NAME: ${name} | PRICE: $${salePrice}`;
...
function createProductItemElement({ id: sku, title: name, thumbnail: image, price: salePrice }) {
  ...
  section.appendChild(createCustomElement('span', 'item__price', `Apenas ${salePrice} reais`));
